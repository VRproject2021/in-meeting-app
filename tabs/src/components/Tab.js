...
import { Avatar, Loader, Button, Label, TextArea } from '@fluentui/react-northstar';
...
class Tab extends React.Component {
  constructor(props) {
    ...
    this.displayInMeetingDialogVR = this.displayInMeetingDialogVR.bind(this);
   ...
   */
  displayInMeetingDialog =  async () => {
    // In-meeting dialogs are triggered via the BotFramework SDK. Send the requst to the api-server.
    let serverURL = `https://account.altvr.com/favorites/favorite?favoritable_id=${this.state.eventId}`;
    if(this.state.eventId == ""){
      alert("Please enter a valid event id");
    }
    else{
      alert("Please put on your headset and join AltspaceVR. Once you’ve signed into AltspaceVR simply go to your “Interested” tab to locate this remote VR session.");
   ...
   * Display an in-meeting dialog (popup).
   */
   displayInMeetingDialogVR() {
    // In-meeting dialogs are triggered via the BotFramework SDK. Send the requst to the api-server.
    // let serverURL = 'https://account.altvr.com/worlds/1827801773956399492/spaces/1827801986817327561';
    ...
    if(this.state.eventId == ""){
      ...
      window.open(`altspace://account.altvr.com/events/${this.state.eventId}`);
    }
  }

  saveEventId(event) {
    let newVal = event.target.value;
    this.setState({eventId: newVal});
    console.log(newVal);
    // alert(`EventId saved:${newVal}`);
  }
  ...
  render() {
    ...
    let title = Object.keys(this.state.context).length > 0 ?
      `Welcome to the VR space` : <Loader />;
      ...
      content =
        <div>
          <MediaQuery maxWidth={280}>
            <h3>{title}</h3>
            Enter eventId: <TextArea type="text" name="txt" value={this.state.eventId} onChange={this.saveEventId} resize={true}/>
            <br/>
            <br/>
            <div class='flex-column'>
            <Button onClick={this.displayInMeetingDialogVR} title={"Requires AltspaceVr to be installed"} primary>2D</Button>
            <Button onClick={this.displayInMeetingDialog} title={"Will require a physical headset"}primary>Headset</Button>
            </div>
          </MediaQuery>
          <MediaQuery minWidth={281}>
            <h1>{title}</h1>
            {/* <Button onClick={this.displayInMeetingDialogWeb} primary>Show bot message</Button> */}
